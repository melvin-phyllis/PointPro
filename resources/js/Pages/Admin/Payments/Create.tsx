    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="mx-auto max-w-xl space-y-6">
                ...
                <h1 className="text-xl font-bold text-primary">Enregistrer un paiement manuel</h1>
                ...
                <form onSubmit={submit} className="space-y-5 rounded-xl border border-theme bg-surface p-6">
                    ...
                    <div>
                        ...
                        {company ? (
                            <div className="rounded-lg border border-theme bg-subtle px-3 py-2 text-sm text-primary">
                                {company.name}
                    ...
                    {/* Plan */}
                    <div>
                        <label className="block text-xs font-medium text-muted mb-2">Plan *</label>
                            ...
                            {plans.map(plan => (
                                <button key={plan.id} type="button" onClick={() => selectPlan(plan.id)}
                                    className={`rounded-lg border p-3 text-left transition ${data.plan_id === plan.id
                                            ...
                                            : 'border-theme bg-white/[0.02] hover:border-white/20'
                                        }`}>
                    ...
                    {/* Montant */}
                    <div>
                        <label className="block text-xs font-medium text-muted mb-1">Montant (FCFA) *</label>
                    ...
                    {/* Méthode de paiement */}
                    <div>
                        <label className="block text-xs font-medium text-muted mb-1">Méthode de paiement *</label>
                    ...
                    {data.payment_method === 'mobile_money' && (
                        <div className="grid gap-3 sm:grid-cols-2">
                            <div>
                                <label className="block text-xs font-medium text-muted mb-1">Numéro de téléphone</label>
                                <input value={data.metadata.phone} onChange={e => setData('metadata', { ...data.metadata, phone: e.target.value })}
                                    className="w-full rounded-lg border border-theme bg-subtle px-3 py-2 text-sm text-primary focus:border-emerald-500 focus:outline-none" />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-muted mb-1">Opérateur</label>
                    ...
                    {data.payment_method === 'bank_transfer' && (
                        <div className="grid gap-3 sm:grid-cols-2">
                            <div>
                                <label className="block text-xs font-medium text-muted mb-1">Banque</label>
                                <input value={data.metadata.bank} onChange={e => setData('metadata', { ...data.metadata, bank: e.target.value })}
                                    className="w-full rounded-lg border border-theme bg-subtle px-3 py-2 text-sm text-primary focus:border-emerald-500 focus:outline-none" />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-muted mb-1">Réf. transaction</label>
                    ...
                    <div>
                        ...
                        <textarea value={data.notes} onChange={e => setData('notes', e.target.value)} rows={2}
                            className="w-full rounded-lg border border-theme bg-subtle px-3 py-2 text-sm text-primary placeholder-gray-500 focus:border-emerald-500 focus:outline-none resize-none" />
                    </div>

                    <div className="flex justify-end gap-3 pt-2">
                        <Link href={route('admin.payments.index')} className="rounded-lg px-4 py-2 text-sm text-muted hover:text-primary">Annuler</Link>
