...
export default function PlansIndex({ plans }: Props) {
    function destroy(plan: Plan) {
        if (confirm(`Supprimer le plan "${plan.name}" ?`)) router.delete(route('admin.plans.destroy', plan.id));
    }
        ...
        <AuthenticatedLayout>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-xl font-bold text-white">Plans tarifaires</h1>
                        <p className="text-sm text-gray-400">{plans.length} plan(s) configuré(s)</p>
                    </div>
                    <Link href={route('admin.plans.create')} className="rounded-lg bg-emerald-500 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-600 transition">
                        + Nouveau plan
                    </Link>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {plans.map(plan => (
                        <div key={plan.id} className={`rounded-xl border p-5 space-y-4 ${plan.is_active ? 'border-white/10 bg-[#0D1117]' : 'border-white/5 bg-white/[0.01] opacity-60'}`}>
                            <div className="flex items-start justify-between">
                                <div>
                                    <h3 className="font-semibold text-white">{plan.name}</h3>
                                    <p className="text-xs text-gray-500">{plan.slug}</p>
                                </div>
                                <span className={`rounded-full px-2 py-0.5 text-xs ${plan.is_active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-gray-500/20 text-gray-400'}`}>
                            ...
                            <div className="flex items-center justify-between border-t border-white/10 pt-3">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => destroy(plan)} className="text-xs text-red-400 hover:underline">Supprimer</button>
                                </div>
                            </div>
                        </div>
                    ))}
